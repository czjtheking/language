...
public class DownLoadThreadPoolUtil {
    ...
    private static volatile DownLoadThreadPoolUtil mInstance;
    ...
    private DownLoadThreadPoolUtil() {
...
//                new DefaultThreadFactory(Thread.NORM_PRIORITY, "downLoadThread-pool-")
//        );
    }

    public static DownLoadThreadPoolUtil getInstance() {
        if (mInstance == null) {
            synchronized (DownLoadThreadPoolUtil.class) {
                if (mInstance == null) {
                    mInstance = new DownLoadThreadPoolUtil();
                }
            }
        }
        return mInstance;
    ...
    public void execute(Runnable runnable) {
        if (executor == null) {
            ...
            System.out.println("线程池容量: " + corePoolSize + "线程池最大容量：" + maxPoolSize);
            executor = Executors.newFixedThreadPool(corePoolSize);
//                    new ThreadPoolExecutor(
//                    corePoolSize,
//                    maxPoolSize,
...
//                    new DefaultThreadFactory(Thread.NORM_PRIORITY, "downLoadThread-pool-"));
    ...
    * 等待执行时间（秒）
     ...
     * 如果等待时间没有超过指定时间，等待
    * */
    public boolean allTaskCompleted(Long timeOut) throws InterruptedException {
